...
if [ -r /usr/share/doc/fzf/examples/key-bindings.zsh ]; then
	...
	if command -V fdfind >/dev/null 2>&1; then
		export FZF_DEFAULT_COMMAND='fdfind -c always --strip-cwd-prefix --hidden --follow --exclude .git --exclude .cache --exclude /proc'
		export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
		ansi=--ansi
	elif command -V fd >/dev/null 2>&1; then
		export FZF_DEFAULT_COMMAND='fd -c always --strip-cwd-prefix --hidden --follow --exclude .git --exclude .cache --exclude /proc'
		export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
		ansi=--ansi
	fi
